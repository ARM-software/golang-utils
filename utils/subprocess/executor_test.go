...
func TestExecuteWithEnvironment(t *testing.T) {
	...
	t.Run("happy", func(t *testing.T) {
		require.NoError(t, ExecuteWithEnvironment(context.Background(), logger, nil, "test sleep", "success", "faillure", "sleep", "2"))
	})
	t.Run("timeout", func(t *testing.T) {
		timeoutCtx, cancel := context.WithTimeout(context.Background(), 2*time.Second)
		defer cancel()
		err = ExecuteWithEnvironment(timeoutCtx, logger, nil, "test sleep with timeout", "success", "failure", "sleep", "5")
		require.Error(t, err)
