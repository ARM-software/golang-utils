...
func TestToSlice(t *testing.T) {
	...
	t.Run("with time", func(t *testing.T) {
		...
		structSlice, err := ToSlice[TestStruct3WithTime](&testStruct)
		require.NoError(t, err)
		newStruct := TestStruct3WithTime{}
		require.NoError(t, FromSlice[TestStruct3WithTime](structSlice, &newStruct))
	...
	t.Run("args", func(t *testing.T) {
		...
		someText := faker.Paragraph()
		now := time.Now().UTC()
		require.NoError(t, FromArgs[TestStruct4](&testStruct, "some_duration", "15m", "test", someText, "struct.some_duration", "5h10m50s", "struct.some_time", now.Format(time.RFC3339Nano)))
		duration, err := time.ParseDuration("5h10m50s")
		require.NoError(t, err)
		assert.Equal(t, duration, testStruct.Struct.Duration)
		assert.Equal(t, 15*time.Minute, testStruct.Duration)
		assert.Equal(t, someText, testStruct.Test)
		assert.WithinDuration(t, now, testStruct.Struct.Time, 0)
	})
